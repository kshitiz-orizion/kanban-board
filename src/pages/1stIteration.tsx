...
// ... your Issue interface and other components remain the same ...
interface Issue {
  id: string;
  title: string;
...
// const initialIssues: Issue[] = IssuesList as Issue[];
...
const DraggableCard = ({
  ...
  const style = {
    transform: transform
      ...
      : undefined,
    cursor: 'grab',
    borderTop: `2px solid ${issue.status === "Done" ? "green" : issue.status === "Backlog" ? "red" : "orange"}`
  };
  ...
  const handleClick = (id: String) => {
    if (!isDraggingGlobal) {
      ...
      navigate(`/issue/${id}`)
      // Your click logic here (open modal, navigate, etc)
    }
  };
  ...
  return (
    ...
    >
      {/* <div className='topBorder' style={{background:'green', height:'2px', position:'sticky', top:'10px'}}></div> */}
      <h4>{issue.title}</h4>
      <p>
  ...
  return (
    <div ref={setNodeRef} className="kanbanColumn">
      ...
      {issues.length > 0 ? (
        ...
        <p style={{ color: '#aaa', fontStyle: 'italic' }}>No matching issues</p>
      )}
    </div>
  );
};
...
export const BoardPage = () => {
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (newStatus) {
      // Save current state before applying drag
      ...
      undoTimer.current = setTimeout(async () => {
        ...
        try {
          ...
          console.log("===inside try=====",previousIssues)
        }
        catch (e) {
          handleUndo()
  ...
  return (
    <div style={{ padding: '1rem' }}>
      ...
      {/* 🛑 Undo Notification */}
      {showUndo && (
        <div className="undo-banner" style={{ marginBottom: '1rem', background: '#fff3cd', padding: '10px', border: '1px solid #ffeeba' }}>
